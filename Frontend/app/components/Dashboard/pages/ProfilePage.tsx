...
function draftToBp(draft: EditableDraft): Partial<BusinessProfile> {
  return {
    ...
    employee_count: draft.employee_count
      ? parseInt(draft.employee_count, 10)
      : 0,
...
export const ProfilePage: React.FC<ProfilePageProps> = ({
  ...
  const completionFields = [
    ...
    businessProfile.business_type !== "lainnya"
              ...
              <Field
                ...
                hint={
                  !draft.kbli_code && isEditing
                    ? "Fitur Scanner bisa bantu rekomendasikan kode KBLI yang sesuai"
                    : undefined
                }
              />
            </div>
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {(
              ...
              ] as { key: keyof BusinessProfile; label: string; desc: string; icon: string; imageKey: string }[]
          ...
          {!isEditing && (
            <p className="text-[11px] text-gray-400 mt-3">
              Klik <span className="font-semibold text-amber-600">Edit Profil</span> di atas untuk memperbarui status dan unggah bukti sertifikat.
            </p>
          )}
        </SectionCard>
      </div>
